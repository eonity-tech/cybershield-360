...
public class DevicePersistenceAdapter implements DeviceRepository {
    ...
    // --- MAPPERS PRIVÉS (Pourraient être dans une classe DeviceMapper séparée) ---
    ...
    private Device toDomain(DeviceEntity entity) {
        ...
        Device domain = new Device(
                ...
                entity.getOsType(),
                entity.getOsVersion(),
                entity.getHostname(), // <-- Ajouté
                entity.getVendor(),   // <-- Ajouté
                entity.getTtl(),      // <-- Ajouté
                entity.getOpenPorts() // <-- Ajouté
        );
